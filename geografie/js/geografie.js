...
$(document).ready(function(){
    ...
    $(".city").on("click", function(){
        if($("#mesta:checked").val()) {
            ...
            $(this).css("fill", "green");
            let id = $(this).attr('id');
            let town = town.find(item => {return item.id == id});
            $("#info").append(`
            <div class="col-12">
                <h2 class="bg-primary">${town.city} 
                    <small class="text-small">${town.peoples} obv.</small>
                </h2>
            </div>
            <div class="col-2">
                <img src="img/${town.sign}" class="img-fluid">
            </div>
            <div class="col-10">
                ${town.text}
            </div>
            `);
            /*$('#info').fadeOut(1000, function() {$('#info').html(`
            <div class="col-12">
                <h2 class="bg-primary">${town.city} 
                    <small class="text-small">${new Intl.NumberFormat('cs-CS').format(town.peoples)} obv.</small>
                </h2>
            </div>
            <div class="col-2">
                <img src="img/${mesto.sign}" class="img-fluid">
            </div>
            <div class="col-10">
                ${town.text}
            </div>`)});
